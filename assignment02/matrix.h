 ...
 * matrix[2][4] = true;
 *
 * Time complexity: O(rc) where r is the number of rows and c is the
 *                  number of columns
 */
 ...
 * Time complexity: O(r) where r is the number of rows of the matrix
 */
void freeBoolMatrix(bool **matrix);

/**
 * Allocates and returns a 2D array/matrix of cell structs with the
 * given number of rows and columns. All entries are initialised to
 * (0, 0). It is the user's responsibility to call `freeCellMatrix` to
 * free the matrix.
 *
 * Time complexity: O(rc) where r is the number of rows and c is the
 *                  number of columns
