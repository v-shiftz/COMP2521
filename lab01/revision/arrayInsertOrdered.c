...
void insertOrdered(int *arr, int size, int value) {
	int i = size - 1; // Start from the last element of the existing array
    while (i >= 0 && arr[i] > value) { // Move elements that are greater than value, one position ahead
        arr[i + 1] = arr[i];
        i--;
    }
    arr[i + 1] = value; // Insert the new value at its correct position
}
