...
void recursiveInsert(struct node **currNode, int value) {
    // Base case: If we reach the end of the list or find the position to insert
    if (*currNode == NULL || (*currNode)->value > value) {
        ...
        newNode->value = value;
        newNode->next = *currNode; // Point new node to the current node or NULL
        *currNode = newNode; // Update the current node (or head) to point to new node
    } else {
        // Recursive case: Move to the next node
        recursiveInsert(&((*currNode)->next), value);
    }
}

void listInsertOrdered(struct list *list, int value) {
	recursiveInsert(&(list->head), value);
}
