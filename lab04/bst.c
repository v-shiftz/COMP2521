...
int bstClosest(struct node *t, int val) {
	...
	int closestNum = t -> value;
	if (closestNum == val){
		return val;
	}
	if(abs(val - closestNum) > abs(val - (t -> value))){
		closestNum = t -> value;
	}
	if (val < t -> value){
		if (t -> left != NULL){
			return bstClosest(t -> left, closestNum);
		}
	}else if (val > t -> value){
		if (t -> right != NULL){
			return bstClosest(t -> right, closestNum);
		}
	}
...
void bstLevelOrder(struct node *t) {
	...
	while (!QueueIsEmpty(q)) {
		struct node *current = QueueDequeue(q);
		showBstNode(current);
