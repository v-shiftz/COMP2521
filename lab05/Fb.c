...
static struct adjNode *removeAdjNode(struct adjNode *node, int v);
bool FbUnfriend(Fb fb, char *name1, char *name2) {
    ...
    if (!inAdjList(fb->adj[id1], id2) || !inAdjList(fb->adj[id2], id1)){
        return false;
    }

    fb->adj[id1] = removeAdjNode(fb->adj[id1], id2);
    fb->adj[id2] = removeAdjNode(fb->adj[id2], id1);
...
static struct adjNode *removeAdjNode(struct adjNode *node, int v){
    ...
    if (node->v == v){
        struct adjNode *temp = node->next;
        free(node);
        return temp;
    }else{
        node->next = removeAdjNode(node->next, v);
        return node;
    }
}
List FbMutualFriends(Fb fb, char *name1, char *name2) {
    ...
    int id1 = nameToId(fb, name1);
    int id2 = nameToId(fb, name2);
    for (struct adjNode *node1 = fb->adj[id1]; node1 != NULL; node1 = node1->next){
        if (inAdjList(fb->adj[id2], node1->v)){
            ListAppend(l, fb->names[node1->v]);
        }
    }
    return l;
}
