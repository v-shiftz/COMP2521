...
void HashTableDelete(HashTable table, int key) {
    ...
    int currentIndex = (index + 1) % table->numSlots;
    while (!table->slots[currentIndex].empty) {
        int rehashedKey = table->slots[currentIndex].key;
        int rehashedValue = table->slots[currentIndex].value;

        table->slots[currentIndex].empty = true;
        table->numItems--;

        HashTableInsert(table, rehashedKey, rehashedValue);

        currentIndex = (currentIndex + 1) % table->numSlots;
    }
}
/////////////////////////////////////////////////////
